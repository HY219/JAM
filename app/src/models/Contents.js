...
class Contents{
    static getContentByCategory(category){
        return new Promise((resolve, reject) => {
            const query = "SELECT * FROM notice WHERE "+category+"= 1 ORDER BY num DESC LIMIT 0,9;";
            db.query(query,
                (err, results)=>{
                if(err) reject(`${err}`);
                resolve(results);
    ...
    static getteamContent(num){
        return new Promise((resolve, reject) => {
            const query = "SELECT * FROM teambuild WHERE notice_num = ? ORDER BY num DESC LIMIT 0, 9;";
            db.query(query,
                [num],
                (err, results)=>{
                if(err) reject(`${err}`);
